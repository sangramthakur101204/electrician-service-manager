...
public class CustomerController {
    ...
    private String buildFooter(Long ownerId) {
        ...
        return settingsRepository.findByOwnerId(ownerId).map(cs -> {
            ...
            if (cs.getCompanyAddress()!= null && !cs.getCompanyAddress().isBlank()) {
                ...
                sb.append("\n🗺️ https://maps.google.com/?q=").append(URLEncoder.encode(cs.getCompanyAddress(), StandardCharsets.UTF_8));
            }
            if (cs.getLinksJson() != null && !cs.getLinksJson().isBlank()) {
                try {
                    com.fasterxml.jackson.databind.ObjectMapper om = new com.fasterxml.jackson.databind.ObjectMapper();
                    var links = om.readValue(cs.getLinksJson(), new com.fasterxml.jackson.core.type.TypeReference<java.util.List<java.util.Map<String,String>>>(){});
                    for (var l : links) {
                        String url = l.get("url"); String lbl = l.get("label");
                        if (url != null && !url.isBlank()) sb.append("\n🔗 ").append(lbl != null && !lbl.isBlank() ? lbl + ": " : "").append(url);
                    }
    ...
    private String compName(Long ownerId) {
        ...
        return settingsRepository.findByOwnerId(ownerId)
                .map(cs -> cs.getCompanyName() != null && !cs.getCompanyName().isBlank() ? cs.getCompanyName() : "Matoshree Enterprises")
    ...
    public ResponseEntity<String> thankYouMessage(@PathVariable Long id, HttpServletRequest req) {
        ...
        return customerRepository.findById(id).map(c -> {
            Long cOwnerId = c.getOwner() != null ? c.getOwner().getId() : ownerId;
            String co = compName(cOwnerId);
            String msg = "✅ *Service Complete — " + co + "*\n\n"
                    + "Hello *" + c.getName() + "*! 👋\n\n"
                    + "Thank you for trusting us with your appliance. "
                    + "Your service has been completed successfully!\n\n"
                    + "🔧 *Service Summary:*\n"
                    + "• Machine: " + c.getMachineType() + " (" + c.getMachineBrand() + (c.getModel()!=null&&!c.getModel().isBlank()?" "+c.getModel():"") + ")\n"
        ...
        return customerRepository.findById(id).map(c -> {
            ...
            if (isPending) {
                msg = "⚠️ *Service Reminder — " + co + "*\n\n"
                        ...
                        + c.getMachineBrand() + (c.getModel()!=null&&!c.getModel().isBlank()?" - "+c.getModel():"") + ") "
